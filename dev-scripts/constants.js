...
export const SUGGEST_TOPICS_PROMPT = `You are a senior social strategist; generate exactly 4 social media suitable topic titles for the {{industry}} industry in British English for Australia - 2 ${EVERGREEN} FAQs and 2 ${TRENDING} items; when web search is available, ground the ${TRENDING} items in Australian interest from the last 90 days and ignore sources older than 120 days; never include years or dates in any title; each title must be 8-12 words, specific, non-overlapping, audience-centred, no hashtags, no em dashes - use single hyphen; output only a 4-line list in the format: ${EVERGREEN}|topic text or ${TRENDING}|topic text, with no extra characters or numbering.`
